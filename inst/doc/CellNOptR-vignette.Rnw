...
Before version 1.3.30, you would type the following commands to create a
CNOlist:
...
data structure into an instance of CNOlist class:
...
displayed by \emph{plotModel} (requires Rgrqphviz to be installed).

<<plotModel, eval=TRUE, fig=FALSE>>=
...
%
<<resSimT2, eval=TRUE, fig=FALSE>>=
cutAndPlot(
    model=ToyNCNOcutCompExp2,
    bStrings=list(ToyT1opt2$bString, ToyT1opt2T2$bString),
    CNOlist=CNOlistToy2,
    plotPDF=TRUE, plotParams=list(cex=0.8, cmap_scale=0.5))
@

\begin{figure*}
\centerline{\includegraphics[width=\textwidth]{Fig7.pdf}}
\caption{output of the cutAndPlot function on the 2-time dat set.}
