...
getFit_BS<-function(
    ...
    if (tPt == 2 && is.null(simResultsT0)==FALSE){
        ...
        r0 <- Diff0^2
        r0 <- as.vector(r0)
        r0 <- r0[ReorderedBS[counter_BS,]] # Bootstrapping part
        
        r <- Diff^2
        # print("Printing -r- before BS"); print(r)
        r <- as.vector(r)
        r <- r[ReorderedBS[counter_BS,]] # Bootstrapping part
        # r <- sample(r,length(r),replace=T) # Bootstrapping part
        # print("Printing -r- after BS"); print(r)
