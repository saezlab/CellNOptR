...
\author{Camille Terfve, Thomas Cokelaer, Aidan MacNamara, Enio Gjerga, 
Attila Gabor, Panuwat Trairatphisan, Julio Saez-Rodriguez}
...
In this section we will demonstrate the use of CellNOptR with the ILP implementation with the example provided in the CellNOptR package. For running this feature, users will have to first obtain an \emph{IBM-CPLEX} license (see \url{https://www.ibm.com/de-de/products/ilog-cplex-optimization-studio}). Onve the \emph{CPLEX} solver is downloaded and installed, then for running CNO-ILP users will have to specify a valid path (\emph{cplexPath}) pointing to the \emph{CPLEX} executable file.
