...
In this section we will demonstrate the use of CellNOptR with the ILP implementation (see \cite{mitsos_2009}) with the example provided in the CellNOptR package.
...
\section{A crossvalidation example with a toy model}

In order to assess the predictive performance of our models and inspect how well our models can predict 
data outside of it’s training set we have implemented cross-validation as another feature within the 
CellNOptR framework.

In this section we will demonstrate the use of the crossvalidation analysis over a toy model.
