...
ilpBinaryTN <- function(cnolist = cnolist, 
                        model = model, 
                        sizeFac = 0.0001, 
                        mipGap = 0, 
                        relGap = 0, 
  ...
  # Function that writes the bounds of the ILP variables
  ...
  # Function that writes the consttraints of the ILP formulation
  ...
  if ((class(cnolist)=="CNOlist")==TRUE){
    writeMIDAS(CNOlist = cnolist, filename = "tempMD.csv", timeIndices = timeIndices, overwrite = TRUE)
