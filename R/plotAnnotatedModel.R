...
#' @title plots network graph with highlighted edges.
#'
...
#' @param model model created by \code{\link{readSIF}} or after \code{\link{preprocessing}}
#' @param CNOlist is a CNOlist object
#' @param bString a sequence made of numbers between 0 and 1 of same length as
...
#' and used.
#' @param indexIntegr additional indices to highlight some edge (optional).
#' @param annotEdgeColors is a NAMED vetor of color values. Use the string vector found in the model$reacID or
#' node1name_k_node2name convention to match the colors to the network.
#' @param annotEdgeMatching one of c("reacID",'reacPar'); depending which convention was used to name the color values
#' @param annotNodeColors is a NAMED vetor of color values for the model nodes. use the NAME of the node.
#' @param signals a list of nodes belonging to the signals class
#' @param stimuli a list of nodes belonging to the stimuli class
#' @param inhibitors a list of nodes belonging to the inhibitors class
#' @param NCNO a list of NCNO nodes.
#' @param compressed a list of compressed nodes
#' @param output the type of output (PNG, PDF, SVG accepted)
#' @param filename the filename (without extension) used to write the dot file
#' @param graphvizParams a list of optional arguments dedicated to Rgraphviz to tune the layout, see \code{\link{plotModel}}
#' @param show show the plot (default is True)
#' @param remove_dot remove the dot file that has been created. Default is TRUE
...
plotAnnotatedModel <- function(model, CNOlist=NULL, bString=NULL, indexIntegr=NULL,
							   ...
							   annotEdgeColors=c(), annotEdgeMatching =c("reacID",'reacPar'),
							   annotNodeColors=NULL){
	# Quick example:
	# ---------------
	...
	# This block is the tricky part of the function. Change with care.
		...
		for (i in 1:dim(reacs)[1]){
			...
			if (length(inputs)==1){
				...
				if(annotEdgeMatching=='reacID'){
					edgeColors[CountReac] = annotEdgeColors[[i]]
				}else edgeColors[CountReac] = "gray50"
				CountReac<-CountReac+1
	...
	orphans = nodes(g)[(graph::degree(g)$inDegree  + graph::degree(g)$outDegree) ==0]
	for (x in orphans){
...
create_layout <- function(g, signals, stimuli){
	...
	if (is.null(signals)==TRUE){
		sinks  <- nodes(g)[graph::degree(g)$outDegree == 0]
	}
	else{
...
createNodeAttrs <- function(g, vertices, stimuli, signals, inhibitors, NCNO,
	...
	for (s in vertices){
		if (length(grep("^and", s))>=1){
			...
			if (graph::degree(g)$inDegree[s]==4){
				#fillcolor[s] = "blue"
				if (mode=="classic"){
					shape[s]="rectangle"
				}
